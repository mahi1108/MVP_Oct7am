package cubex.mahesh.mvp_oct7am.model

import android.content.ContentValues
import android.content.Context
import android.database.sqlite.SQLiteDatabase
import cubex.mahesh.mvp_oct7am.beans.IncExpBean
import cubex.mahesh.mvp_oct7am.presenter.PresenterAPI
import cubex.mahesh.mvp_oct7am.view.MainActivity
import cubex.mahesh.mvp_oct7am.view.ViewAPI

class IncExpModel:PresenterAPI {

    var api:ViewAPI? = null
    var dBase:SQLiteDatabase? = null
    constructor( api:ViewAPI){    // ViewAPI api = MainActivity( ) upcasting
            this.api   = api
            var mActivity = api as MainActivity
        // Downcasting,           MainActivity mActivity = (MainActivity)api;
            dBase = mActivity.openOrCreateDatabase("incexp_db",
                    Context.MODE_PRIVATE,null)

            dBase!!.execSQL("create table if not exists manage_incexp(_id integer primary key autoincrement,date varchar(100),money number,_desc varchar(500),type varchar(100))")

    }

    override fun insertIncExp(bean: IncExpBean) {

        var cv = ContentValues( )
        cv.put("date",bean.date)
        cv.put("money",bean.money)
        cv.put("_desc",bean.desc)
        cv.put("type",bean.type)

        var status =   dBase!!.insert("manage_incexp",
             null,cv)
    if(status != -1.toLong()){
        api!!.insertIncExp("Data  Inserted")
    }else{
        api!!.insertIncExp("Data Insertion is Failed...")
    }
    }

    override fun readIncExp() {
            var c = dBase!!.query("manage_incexp",null,
                    null, null,
                    null,null,null)
            var list = mutableListOf<IncExpBean>()
        while (c.moveToNext()){
            var bean = IncExpBean(c.getString(1),c.getInt(2),
                    c.getString(3), c.getString(4))
            list.add(bean)
        }
        api!!.readIncExp(list)
    }

}